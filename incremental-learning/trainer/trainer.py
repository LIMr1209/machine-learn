...
class Trainer(GenericTrainer):
    def __init__(self, trainDataIterator, testDataIterator, dataset, model, args, optimizer, ideal_iterator=None):
        ...
        self.dynamic_threshold = np.ones(self.dataset.classes, dtype=np.float64)  # 动态阈值
        self.gradient_threshold_unreported_experiment = np.ones(self.dataset.classes, dtype=np.float64)  # 梯度阈值未报告实验

    def update_lr(self, epoch):
        for temp in range(len(self.args.schedule)):
    ...
    def randomly_init_model(self):  # 随机初始化模型
    ...
    def train(self, epoch):
        ...
        for data, y, target in tqdm(self.train_data_iterator):
            ...
            for elem in range(self.args.unstructured_size):
                oldClassesIndices = oldClassesIndices + (target == elem).int()

            old_classes_indices = torch.squeeze(torch.nonzero((oldClassesIndices > 0)).long())  # 旧类索引
            new_classes_indices = torch.squeeze(torch.nonzero((oldClassesIndices == 0)).long())  # 新类索引

            self.optimizer.zero_grad()

            target_normal_loss = target[new_classes_indices]  # 目标正常损失
            data_normal_loss = data[new_classes_indices]  # 数据正常损失

            target_distillation_loss = y.float()  # 目标蒸馏损失
            data_distillation_loss = data  # 数据蒸馏损失
            ...
            # 网络结构弄成1000类?,不够的类numpy 用0 填充 50类训练一次 ??
            ...
            if self.args.no_distill:
                ...
                pred2 = self.model_fixed(data_distillation_loss, T=myT, labels=True).data
                # Softened output of the model
                if myT > 1:
                    output2 = self.model(data_distillation_loss, T=myT)
                else:
                    output2 = output

                self.dynamic_threshold += (np.sum(pred2.cpu().numpy(), 0)) * (
                        myT * myT) * self.args.alpha
                loss2 = F.kl_div(output2, pred2)
