...
class Classifier:
    def __init__(self, **kwargs):
        ...
        self.criterion = t.nn.CrossEntropyLoss().to(opt.device)
    ...
    def write_err_img(self, err_img):
        with open(self.opt.error_img, 'w', newline='') as f:
    ...
    def train_load_model(self):
        ...
        self.model.to(opt.device)  # 加载模型到 GPU
    ...
    def load_compress(self):
        ...
        if self.opt.compress:
            self.compression_scheduler = distiller.file_config(self.model, self.optimizer, opt.compress,
                                                               self.compression_scheduler)  # 加载模型修剪计划表
            self.model.to(opt.device)

    def visualization_train(self, input, train_losses):
        if opt.vis:
            self.vis.img('image', (input.data.cpu() * 0.225 + 0.45).clamp(min=0, max=1))  # 查看数据增强图片
            self.vis.plot('loss', train_losses.val)  # 绘图

    def visualization_test(self, input, labels, results):
        if opt.vis:
            self.vis.img('image', (input.data.cpu() * 0.225 + 0.45).clamp(min=0, max=1))  # 查看数据增强图片
            self.vis.log('labels:' + ','.join([self.opt.cate_classes[i] for i in labels.tolist()]))  # 正确标签
            self.vis.log('results:' + ','.join([self.opt.cate_classes[i] for i in results.tolist()]))  # 识别标签
        ...
        for epoch in range(self.start_epoch, self.opt.max_epoch):
            ...
            for ii, (data, labels, img_path) in enumerate(self.train_dataloader):
                ...
                if (ii + 1) % opt.print_freq == 0:
                    self.visualization_train(input, self.train_losses)
            ...
            val_loss, val_top1, val_top5 = val(self.model, self.criterion, self.val_dataloader, epoch)  # 校验模型
    ...
    def test(self):
        ...
        for ii, (data, labels, img_path) in tqdm(enumerate(self.test_dataloader)):
            ...
            total += input.size(0)
            self.visualization_test(input, labels, results)
    ...
    def sensitivity(self):
        self.load_data()
        self.load_model()
        sensitivities = np.arange(self.opt.sensitivity_range[0], self.opt.sensitivity_range[1], self.opt.sensitivity_range[2])
        return sensitivity_analysis(self.model, self.criterion, self.test_dataloader, self.opt, sensitivities, msglogger)
