...
应用程序从Torchvision的ImageNet分类中借用其主要流代码。

培训示例应用程序（https://github.com/pytorch/examples/tree/master/imagenet）。

为了使它熟悉易懂，我们试图保持它的相似性。



集成压缩非常简单：只需添加适当的调用

压缩\调度程序回调，针对培训的每个阶段。培训框架

看起来像下面的伪代码。锅炉盘割炬分级培训

在compressionScheduler的调用中出现斑点。



每一个时代：

压缩调度程序。在epoch开始（epoch）
...
此示例应用程序可用于Torchvision的ImageNet图像分类

模型或提供的样本模型：



-CIFAR的resnet:https://github.com/junyuseu/pytorch-cifar-models

-用于ImageNet的MobileNet:https://github.com/marvis/pytorch-mobilenet
'''
import math
import time
import os
...
from dataset import datasets_fn
...
def main():
    ...
    if args.deterministic:
        ...
        if args.workers > 1:
            msglogger.error(
                'ERROR: Setting --deterministic requires setting --workers/-j to 0 or 1')  # 错误：设置--确定性要求将--workers/-j设置为0或1
            exit(1)  # 正常退出程序
    ...
    if args.qe_calibration:
        ...
        activations_collectors.update(create_quantization_stats_collector(model)) # 量化统计收集器
        args.evaluate = True
    ...
    train_loader, val_loader, test_loader, _ = get_data_loaders(datasets_fn, r'/home/tian/Desktop/image_yasuo',
                                                                args.batch_size,
    ...
    if args.thinnify:
        ...
        print(
            "Note: your model may have collapsed to random inference, so you may want to fine-tune")  # 注意：您的模型可能已折叠为随机推理，因此您可能需要对其进行微调。
        return
...
def evaluate_model(model, criterion, test_loader, loggers, activations_collectors, args, scheduler=None): # 校验模型
...
def summarize_model(model, dataset, which_summary): # 模型概述输出
...
def automated_deep_compression(model, criterion, optimizer, loggers, args):  # 自动化的深层压缩
    train_loader, val_loader, test_loader, _ = get_data_loaders(datasets_fn, r'/home/tian/Desktop/image_yasuo',
                                                                args.batch_size,
                                                                args.workers, args.validation_split, args.deterministic,
                                                                args.effective_train_size, args.effective_valid_size,
...
def greedy(model, criterion, optimizer, loggers, args):  # 贪婪压缩
